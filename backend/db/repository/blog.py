...
from backend.db.schema.blog import CreateBlog, UpdateBlog
...
def update_blog_by_id(id : int, blog: UpdateBlog, db: Session, author_id: int):
    blog_in_db: Blog = db.query(Blog).filter(Blog.id==id).first()
    if not blog_in_db:
        return {
    ...
    if not blog_in_db.author_id == author_id:
        return {
            "error": f"Only the author can modify the blog"
        }
